...
%%
 ...
 %% Baseline erp
    ...
    for trial=1:length(allTrials) % get rid of last trigger?
        ...
        master_matrix_R(current:current+(length(tr)-1),2) = total_numtr;
        %% 3. inter-subject trial number
        master_matrix_R(current:current+(length(tr)-1),3) = trial;
        %% 4. Time:
         master_matrix_R(current:current+(length(tr)-1),4)=tr; 
        %% 5. Stim Locked CPP
        master_matrix_R(current:current+(length(tr)-1),5)=erpr(ch_CPP,:, trial);
