...
allsubj = {'LK','AR','MH','AA','NT','OF','RO','PR','AS','OM', ...
    ...
    'SH','HP1M','BR2M','JK3M','SJ4M','CB5M','MO6M','CS7M','EZ8M','KS9M', ...clos
...
for s=1:length(allsubj)
    ...
    % beta Spectrotemporal Evolution a la Thut 
    beta_TSE = []; beta_asym = [];
    for trial = 1:size(erp,3)
        ...
        beta_asym(right_hemi,:,trial) = (beta_TSE(right_hemi,:,trial)-beta_TSE(left_hemi,:,trial))./...
            ((beta_TSE(right_hemi,:,trial)+beta_TSE(left_hemi,:,trial))/2);
    end
...
%     RT_index(s) = (RT_all_zs(s,1)-RT_all_zs(s,2))/((RT_all_zs(s,1)+RT_all_zs(s,2))/2);
    ...
    for side = 1:2
        ...
        alpha_side(s,:,:,side) = squeeze(mean(beta_TSE(1:numch,:,[conds{s,:,side}]),3));
        alpha_base_side(s,:,:,side) = squeeze(mean(beta_TSE_base(1:numch,:,[conds{s,:,side}]),3));
        beta_asym_side(s,:,:,side) = squeeze(mean(beta_asym(1:numch,:,[conds{s,:,side}]),3));
        beta_asym_avg_side(s,right_hemi,:,side) = (alpha_side(s,right_hemi,:,side)-alpha_side(s,left_hemi,:,side))./...
            ((alpha_side(s,right_hemi,:,side)+alpha_side(s,left_hemi,:,side))/2);
            
        for iti = 1:3
            RT_temp = RTs{s,iti,side};
            beta_asym_temp = squeeze(mean(mean(beta_asym(ch_beta_asym,find(beta_t>=alpha_t1 & beta_t<=alpha_t2),[conds{s,iti,side}]),1),2));
            alpha_temp = squeeze(mean(mean(beta_TSE(ch_beta_asym,find(beta_t>=alpha_t1 & beta_t<=alpha_t2),[conds{s,iti,side}]),1),2));
...
figure
plottopo(stat_values(:,:,1),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
...
figure
plottopo(stat_values(:,:,2),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
    0  0.05], ...
    'title',['beta vs RT asym p-values'],'legend',side_tags,'showleg','off','ydir',1)
...
for tt = 1:length(beta_t)
    ...
    title(['beta F-values: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
for tt = 1:length(beta_t)
    ...
    topoplot(plot_mean,chanlocs,'maplimits', ...
        [0 0.05], ...
        'electrodes','off','plotchans',plot_chans);
    title(['beta p-values: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
rm = fitrm(tab,[bins_names{1},'-',bins_names{end},'~1'],'WithinDesign',within); % ,'Y1-Y3~1'
ranovatbl = ranova(rm);
disp(['Stim-locked beta asym bins: F = ',num2str(ranovatbl{1,4}),', p = ',num2str(ranovatbl{1,5})])
...
SEM = std(squeeze(mean(mean(RT_asym_bins_mean_all(:,[57],find(beta_t>=499 & beta_t<=549),:),3),2)),[],1)/sqrt(length(allsubj));
figure
h = errorbar(squeeze(mean(mean(mean(RT_asym_bins_mean_all(:,[57],find(beta_t>=499 & beta_t<=549),:),3),2),1)),SEM);
h.LineWidth = 2;

figure
plottopo(stat_values(:,:,1),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
    min(min(min(stat_values(plot_chans,:,1))))  max(max(max(stat_values(plot_chans,:,1))))], ...
    'title',['beta asym vs RT asym F-values'],'legend',side_tags,'showleg','off','ydir',1)
figure
plottopo(stat_values(:,:,2),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
    0  0.05], ...
    'title',['beta asym vs RT asym p-values'],'legend',side_tags,'showleg','off','ydir',1)
...
for tt = 1:length(beta_t)
    ...
    topoplot(plot_mean,chanlocs,'maplimits', ...
        ...
        max(max(stat_values(:,:,1)))], ...
        'electrodes','off','plotchans',plot_chans);
    title(['beta asym F-values: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
for tt = 1:length(beta_t)
    ...
    topoplot(plot_mean,chanlocs,'maplimits', ...
        [0 0.05], ...
        'electrodes','off','plotchans',plot_chans);
    title(['beta asym p-values: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
figure
plottopo(alpha_group_side(:,:,:),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
...
t1 = -500; t2 = -50;
figure
plot_mean = squeeze(mean(mean(alpha_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),2),3));
topoplot(plot_mean,chanlocs,'maplimits', ...
...
figure
plottopo(alpha_base_group_side(:,:,:),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
...
for side = 1:2    
    ...
    for tt = 1:size(time_windows,2)
        plot_mean = squeeze(mean(alpha_base_group_side(:,find(beta_t>=time_windows(1,tt) & beta_t<=time_windows(2,tt)),side),2));
        subplot(3,3,tt)
        topoplot(plot_mean,chanlocs,'maplimits', ...
            [min(min(min(alpha_base_group_side(:,find(beta_t>time_windows(1,1) & beta_t<time_windows(2,end)),:))))...
            max(max(max(alpha_base_group_side(:,find(beta_t>time_windows(1,1) & beta_t<time_windows(2,end)),:))))], ...
            'electrodes','off','plotchans',plot_chans);
...
figure
for side = 1:2
    plot_mean = squeeze(mean(alpha_base_group_side(:,find(beta_t>=t1 & beta_t<=t2),side),2));
    subplot(1,2,side)
    topoplot(plot_mean,chanlocs,'maplimits', ...
        [min(squeeze(mean(mean(alpha_base_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),3),2))) ...
        max(squeeze(mean(mean(alpha_base_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),3),2)))], ...
        'electrodes','off','plotchans',plot_chans);    
    colorbar('FontSize',12)
...
figure
plottopo(beta_asym_group_side(:,:,:),'chanlocs',chanlocs,'limits',[beta_t(1) beta_t(end) ...
...
for side = 1:2    
    figure
    for tt = 1:size(time_windows,2)
        plot_mean = squeeze(mean(beta_asym_group_side(:,find(beta_t>=time_windows(1,tt) & beta_t<=time_windows(2,tt)),side),2));
        subplot(3,3,tt)
        topoplot(plot_mean,chanlocs,'maplimits', ...
            [min(min(min(beta_asym_group_side(:,find(beta_t>time_windows(1,1) & beta_t<time_windows(2,end))))))...
            max(max(max(beta_asym_group_side(:,find(beta_t>time_windows(1,1) & beta_t<time_windows(2,end))))))], ...
            'electrodes','off','plotchans',plot_chans);
...
t1 = -500; t2 = -50;
figure
plot_mean = squeeze(mean(mean(beta_asym_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),2),3));
topoplot(plot_mean,chanlocs,'maplimits', ...
    [min(squeeze(mean(mean(beta_asym_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),3),2))) ...
    max(squeeze(mean(mean(beta_asym_group_side(:,find(beta_t>=t1 & beta_t<=t2),:),3),2)))], ...
    'electrodes','off','plotchans',plot_chans);
colorbar('FontSize',12)
...
beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_side(:,ch_beta_asym,find(beta_t>=t1 & beta_t<=t2),:),2),3),4))';
% beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_avg_side(:,ch_beta_asym,find(beta_t>=t1 & beta_t<=t2),:),2),3),4))';
alpha_zscores = zscore(beta_asym_pretarg);
...
alpha_corr_topo=[];
for chan = right_hemi 
    beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_side(:,chan,find(beta_t>=t1 & beta_t<=t2),:),2),3),4))';
%     beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_avg_side(:,chan,find(beta_t>=t1 & beta_t<=t2),:),2),3),4))';
    alpha_zscores = zscore(beta_asym_pretarg);
    RT_zscores = zscore(RT_index);
    indx = find(abs(alpha_zscores)<z_thresh & abs(RT_zscores)<z_thresh);
...
for tt = 1:length(beta_t)
    ...
    topoplot(plot_mean,chanlocs,'maplimits', ...
        ...
        'electrodes','off','plotchans',plot_chans);
    title(['beta asym corr: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
for tt = 1:length(beta_t)
    ...
    topoplot(plot_mean,chanlocs,'maplimits', ...
        ...
        'electrodes','off','plotchans',plot_chans);
    title(['beta asym DAT1: ',num2str(beta_t(tt)),' ms']);
    colorbar
end
...
for side = 1:2
    alpha_corr_topo=[];
    for chan = right_hemi
        beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_side(:,chan,find(beta_t>=t1 & beta_t<=t2),side),2),3),4))';
        %     beta_asym_pretarg = squeeze(mean(mean(mean(beta_asym_avg_side(:,chan,find(beta_t>=t1 & beta_t<=t2),:),2),3),4))';
        alpha_zscores = zscore(beta_asym_pretarg);
%         RT_zscores = zscore(RT_index);
        indx = find(abs(alpha_zscores)<z_thresh);% & abs(RT_zscores)<z_thresh);
...
for side = 1:2
    alpha_corr_topo=[];
    for chan = plot_chans
        alpha_pretarg = squeeze(mean(mean(mean(alpha_side(:,chan,find(beta_t>=t1 & beta_t<=t2),side),2),3),4))';
...
for s = 1:size(allsubj,2)
    for hemi=1:2
        pre_alpha_hemi(s,hemi)=squeeze(mean(mean(mean(alpha_side(s,pre_alpha_chans(hemi,:),1:find(beta_t==-1),:),2),3),4));
    end
end
...
for s = 1:size(allsubj,2)
    for side=1:2
        for hemi=1:2
            if hemi==1
            post_alpha_lefthemi(s,side)=squeeze(mean(mean(alpha_side(s,pre_alpha_chans(hemi,:),find(beta_t>150 & beta_t<700),side),2),3));
            else
            post_alpha_righthemi(s,side)=squeeze(mean(mean(alpha_side(s,pre_alpha_chans(hemi,:),find(beta_t>150 & beta_t<700),side),2),3)); 
            end
        end
    end
end
