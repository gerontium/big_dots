...
STFTr = zeros(size(STFT,1),length(STFT_timer),size(STFT,3));    
    ...
    for trial=1:length(allTrials) % get rid of last trigger?
        ...
        ID_vector(current:current+(length(STFT_timer)-1)) = subject_folder(s);
        %% 1. Subject number:
        master_matrix_R(current:current+(length(STFT_timer)-1),1) = s;
        %% 2. total trial number:
        master_matrix_R(current:current+(length(STFT_timer)-1),2) = total_numtr;
...
csvwrite (['master_matrix_R_Resp_locked_beta.csv'],master_matrix_R)
cell2csv ('ID_vector_Resp_locked_beta.csv',ID_vector)
