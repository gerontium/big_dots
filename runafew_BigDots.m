...
% path = 'D:\Participant Folders_new\'; %TCD Laptop
%%
...
for s=file_start:length(allsubj)
    ...
    %% For current source density (CSD) tansform:
    if ismember(subject_folder{s},TCD_bigdots)
        E = textread('chans64.asc','%s'); %TCD 64 chan montage
    elseif ismember(subject_folder{s},Monash_bigdots)
        E = textread('chans65_monash.asc','%s'); %Monash 65 chan Acticap montage
    end
    M = ExtractMontage('10-5-System_Mastoids_EGI129.csd',E);  % reading in the montage for the CSD toolbox
    %% Now call the script to extract ERP epochs:
    ScriptToExtractBigDotsEpochs %Choose a better name when you make it Ger!! 
    %%
    end_time = toc;
end
close(h)