...
%     % zscore inside trial
%     erp_z = zeros(size(erp));
%     erpr_z = zeros(size(erpr));
%     CPP_temp = zeros(size(erp,2),size(erp,3));
%     CPPr_temp = zeros(size(erpr,2),size(erpr,3));
%     N2c_temp = zeros(size(erp,2),size(erp,3));
%     N2i_temp = zeros(size(erp,2),size(erp,3));
%     for side = 1:2
%         for iti = 1:3
%             all_trials = [conds{s,iti,side}];
%             for trial = 1:length(all_trials)
%                 for chan = 1:numch
%                     erp_z(chan,:,all_trials(trial)) = zscore(erp(chan,:,all_trials(trial)),[],2);
%                     erpr_z(chan,:,all_trials(trial)) = zscore(erpr(chan,:,all_trials(trial)),[],2);
%                 end
%                 CPP_temp(:,all_trials(trial)) = zscore(squeeze(mean(erp(ch_CPP,:,all_trials(trial)),1)));
%                 CPPr_temp(:,all_trials(trial)) = zscore(squeeze(mean(erpr(ch_CPP,:,all_trials(trial)),1)));
%                 N2c_temp(:,all_trials(trial)) = zscore(squeeze(mean(erp(ch_lr{side},:,all_trials(trial)),1)));
%                 N2i_temp(:,all_trials(trial)) = zscore(squeeze(mean(erp(ch_rl{side},:,all_trials(trial)),1)));
%             end
%         end
%     end
%     
%     % Baseline erp_z, erpr_z
%     baseline_erp = mean(erp_z(:,find(t>=BL_erp(1) & t<=BL_erp(2)),:),2);
%     erp_z = erp_z-repmat(baseline_erp,[1,size(erp_z,2),1]); % baseline full erp
%     erpr_z = erpr_z-repmat(baseline_erp,[1,size(erpr_z,2),1]); % baseline full erpr
%     
%     for side = 1:2
%         ERP_side(s,:,:,side) = squeeze(mean(erp_z(1:numch,:,[conds{s,:,side}]),3));
%         ERPr_side(s,:,:,side) = squeeze(mean(erpr_z(1:numch,:,[conds{s,:,side}]),3));
%     end
%     
%     % Baseline CPP_temp etc
%     baseline = mean(CPP_temp(find(t>=BL_erp(1) & t<=BL_erp(2)),:),1);
%     CPP_temp = CPP_temp-repmat(baseline,[size(CPP_temp,1),1]);
%     baseline = mean(CPP_temp(find(t>=BL_erp(1) & t<=BL_erp(2)),:),1);
%     CPPr_temp = CPPr_temp-repmat(baseline,[size(CPPr_temp,1),1]);
%     baseline = mean(N2c_temp(find(t>=BL_erp(1) & t<=BL_erp(2)),:),1);
%     N2c_temp = N2c_temp-repmat(baseline,[size(N2c_temp,1),1]);
%     baseline = mean(N2i_temp(find(t>=BL_erp(1) & t<=BL_erp(2)),:),1);
%     N2i_temp = N2i_temp-repmat(baseline,[size(N2i_temp,1),1]);
%     
%     for side = 1:2
%         CPP_side(s,:,side) = squeeze(mean(CPP_temp(:,[conds{s,:,side}]),2));
%         CPPr_side(s,:,side) = squeeze(mean(CPPr_temp(:,[conds{s,:,side}]),2));
%         N2c_side(s,:,side) = squeeze(mean(N2c_temp(:,[conds{s,:,side}]),2));
%         N2i_side(s,:,side) = squeeze(mean(N2i_temp(:,[conds{s,:,side}]),2));
%     end
    
%     % zscore inside subject and side
%     for side = 1:2
%         ERP_side(s,:,:,side) = squeeze(mean(erp(1:numch,:,[conds{s,:,side}]),3));
%         ERPr_side(s,:,:,side) = squeeze(mean(erpr(1:numch,:,[conds{s,:,side}]),3));
%         
%         CPP_side(s,:,side) = zscore(squeeze(mean(mean(erp(ch_CPP,:,[conds{s,:,side}]),1),3)));
%         CPPr_side(s,:,side) = zscore(squeeze(mean(mean(erpr(ch_CPP,:,[conds{s,:,side}]),1),3)));
%         N2c_side(s,:,side) = zscore(squeeze(mean(mean(erp(ch_lr{side},:,[conds{s,:,side}]),1),3)));
%         N2i_side(s,:,side) = zscore(squeeze(mean(mean(erp(ch_rl{side},:,[conds{s,:,side}]),1),3)));
%     end

    % zscore inside subject
    for side = 1:2
        ERP_side(s,:,:,side) = squeeze(mean(erp(1:numch,:,[conds{s,:,side}]),3));
        ERPr_side(s,:,:,side) = squeeze(mean(erpr(1:numch,:,[conds{s,:,side}]),3));
    ...
    CPP_cat = zscore(cat(2,CPP_side(s,:,1),CPP_side(s,:,2))); 
    CPP_cat = reshape(CPP_cat,[size(CPP_side(s,:,:))]);
    baseline = mean(CPP_cat(1,find(t>=BL_erp(1) & t<=BL_erp(2)),:),2);
    CPP_side(s,:,:) = CPP_cat-repmat(baseline,[1,size(CPP_cat,2),1]);
    
    CPPr_cat = zscore(cat(2,CPPr_side(s,:,1),CPPr_side(s,:,2))); 
    CPPr_cat = reshape(CPPr_cat,[size(CPPr_side(s,:,:))]);
    CPPr_side(s,:,:) = CPPr_cat-repmat(baseline,[1,size(CPPr_cat,2),1]);
    
    N2c_cat = zscore(cat(2,N2c_side(s,:,1),N2c_side(s,:,2))); 
    N2c_cat = reshape(N2c_cat,[size(N2c_side(s,:,:))]);
    baseline = mean(N2c_cat(1,find(t>=BL_erp(1) & t<=BL_erp(2)),:),2);
    N2c_side(s,:,:) = N2c_cat-repmat(baseline,[1,size(N2c_cat,2),1]);
    
    N2i_cat = zscore(cat(2,N2i_side(s,:,1),N2i_side(s,:,2))); 
    N2i_cat = reshape(N2i_cat,[size(N2i_side(s,:,:))]);   
    baseline = mean(N2i_cat(1,find(t>=BL_erp(1) & t<=BL_erp(2)),:),2);
    N2i_side(s,:,:) = N2i_cat-repmat(baseline,[1,size(N2i_cat,2),1]);
end
