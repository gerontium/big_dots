...
data$IDbyITI<-interaction(data$ID, data$ITI)
#calculate mean and sd 
m <- tapply(data$log_RT,data$IDbyITI,mean)
s <- sqrt(tapply(data$log_RT,data$IDbyITI,var))
#calculate log_RT.Z and save it inside data.frame
data$log_RT.Z <- (data$log_RT-m[data$IDbyITI])/s[data$IDbyITI]
#check that Z scores have mean=0 and std=1 
log_RT.Z_checker <- ddply(data, c("ID", "ITI"), summarise,
               N    = length(log_RT.Z ),
...
data<-data[!abs(data$log_RT.Z)>2.5,]
...
#non10_10_repeat participants have significant leftward RT asymmetry, and the 10_10_repeat participants do not
```{r, echo=FALSE, warning=FALSE}
...
#However, the difference in RT-asymmetry between the 10_10_repeat and non10_10_repeat DAT1_3UTRs is not significant:
...
RT_random_effects_only<-lmer(log(RT) ~ 1 + (1 | DAT1_3UTR/ID) +(1|ITI) + (1|Trial), data = data2, REML=FALSE, na.action = na.omit)
RT_Hemifield<-update(RT_random_effects_only, .~. + Hemifield)
