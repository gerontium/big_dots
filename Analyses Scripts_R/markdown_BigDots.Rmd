...
#Run the Shapiro-Wilk test of normality on the RT, CPPonset, N2c, etc. measures 
...
print("Factorial Permutation test for the effect of Hemifield on SResp Locked Beta Slope:")
print(Hemifield_Perm);
...
##Try reading in a .png and plotting it on a ggplot
library(png)
library(grid)
#Read in pmg
img<-readPNG("ROIs.png")
#transform it to a raster grob
...
png("Correlation betweem RT_Asymmetry.png",  width = 10*600, height = 10*600, units = "px", res = 600)
corPlot(cor_matrix, numbers=T, main="Correlation betweem RT_Asymmetry & SLF_Asymmetry",  show.legend=F, upper=T,  cex = 1)
dev.off()
corPlot(cor_matrix, numbers=T, main="Correlation betweem RT_Asymmetry",  show.legend=F, upper=T,  cex = 1)
...
tidy(anova(final_model)) # anova table 
...
bootResults<-boot(statistic = bootReg, formula = RT_Asym ~  Location + Sex + Age + PreAlphaAsym + N2c_Asym + N2c_latency_Asym + CPPonset_Asym + CPPslope_Asym + Beta_Asym, data = participant_level, R = 2000)
