...
data_Stim_locked_Beta<- data_Stim_locked_Beta %>% filter(!Art_neg100_100PR)
###########################################################################################################
...
ggplot(plotdata_Resp_locked_Beta, aes(x=Time, y=Resp_locked_Beta, color=Hemifield,fill=Hemifield)) + 
    geom_line(size=1.4) + geom_ribbon(aes(ymin=Resp_locked_Beta-ci, ymax=Resp_locked_Beta+ci), alpha = 0.3, colour=NA) + 
        geom_hline(yintercept=0, alpha = 0.5) + geom_vline(xintercept=0, alpha = 0.5) +   
    coord_cartesian(ylim = c(-0.1, 0.01),  xlim = c(-400, 100)) +
    xlab("Time") + ylab("Beta Power (uV)") +
    theme(axis.title.x = element_text(face="bold", size=12),
          axis.text.x  = element_text(face="bold", angle=0,  size=12)) +
    theme(axis.title.y = element_text(face="bold", size=12),
...
print("Factorial Permutation test for the effect of Hemifield on Stim Locked Beta:")
print(Hemifield_Perm);
...
###################################################################
Perm_data<- participant_level %>% 
    select(ID, Beta_Baselined_LeftTarget, Beta_Baselined_RightTarget_Baselined) %>%
    na.omit() %>%
    gather(Hemifield, dv, -ID) 
...
print("Factorial Permutation test for the effect of Hemifield on Baselined Stim Locked Beta:")
print(Hemifield_Perm);

###################################################################

Perm_data<- participant_level %>% 
    select(ID, Beta_Baselined_slope_LeftTarget, Beta_Baselined_slope_RightTarget) %>%
    na.omit() %>%
    gather(Hemifield, dv, -ID) 
...
print("Factorial Permutation test for the effect of Hemifield on Baselined Resp Locked Beta Slope:")
print(Hemifield_Perm);
...
#Re order the columns so that "RT_Asym comes first"
refcols <- c("RT_Asym")
data_for_cor <- data_for_cor[, c(refcols, setdiff(names(data_for_cor), refcols))]
...
ggplot(data_for_cor, aes(x=RT_Asym, y=N2c_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region

ggplot(data_for_cor, aes(x=RT_Asym, y=N2c_latency_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region
...
ggplot(data_for_cor, aes(x=RT_Asym, y=Beta_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region

ggplot(data_for_cor, aes(x=Beta_Asym, y=Beta_slope_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region

ggplot(data_for_cor, aes(x=Beta_slope_Asym, y=N2c_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region

ggplot(data_for_cor, aes(x=Beta_slope_Asym, y=N2c_latency_Asym)) +
    geom_point(size=3) +    # Use hollow circles
    geom_smooth(method=lm,   # Add linear regression line
                se=FALSE)    # Don't add shaded confidence region
...
anova(intercept_only, Controls, PreAlphaAsym, N2c_Asym, N2c_latency_Asym, CPPonset_Asym, CPPslope_Asym, Beta_Asym)
...
# install.packages(c("QuantPsyc","car","leaps"))
library(QuantPsyc)
library(car)
lm.beta(final_model)
...
# Stepwise Regression
library(MASS)
fit <- lm(RT_Asym~ Location + Sex + Age + PreAlphaAsym + N2c_Asym + N2c_latency_Asym + CPPonset_Asym + CPPslope_Asym + Beta_Asym, data=participant_level)
...
library(leaps)
leaps<-regsubsets(RT_Asym~ Location + Sex + Age + PreAlphaAsym + N2c_Asym + N2c_latency_Asym + CPPonset_Asym + CPPslope_Asym + Beta_Asym, data=participant_level,nbest=9)
