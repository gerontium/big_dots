...
SLF<-merge(SLF, Demographics , by.x = "ID", by.y = "ID") 


TVA_ID <- read_excel("TVA ID.xlsx") 
TVA_ID<-TVA_ID[,1:2]


SLF<-merge(SLF, TVA_ID , by = "ID") 
...
######Calculate RT asymmetry Index and add it to "participant_level"######
...
RT_collapsed<- data %>% 
            ...
            # %>% #next add in the ERP measures:
            # left_join(participant_level, by="ID") %>% #next add in the SLF data: 
            # left_join(SLF, by="ID") #DON'T WORRY ABOUT "coercing to character vector" it's just changing ID from a factor class to an character class

#Add in the ERP measures
RT_collapsed<-merge(RT_collapsed, participant_level, by.x = "ID", by.y = "ID") 
...
#use lapply to do t-test on every column 
lapply(select(Asym_data_wide, contains("_LI_"), contains("Asym")), function(x) t.test(x ~ Asym_data_wide$DAT1_3UTR, na.action=na.omit))
#Save it into a data frame
t_test_results<-as.data.frame(sapply(select(Asym_data_wide, contains("_LI_"), contains("Asym")), function(x) t.test(x ~ Asym_data_wide$DAT1_3UTR, na.action=na.omit)))

#Convert t-test results data frame into tidy format
...
#Test relationship between N2_Asym, CPP_Asym, RT_Asym and SLF (I, II add III) Asym

```{r, echo=FALSE, warning=FALSE}

##Now grab only the Asym/lateralistion index (_LI_) measures 
data_for_cor<-select(Asym_data_wide, -ID, -DAT1_3UTR)

##Re order the columns
# data_for_cor <- select(data_for_cor, RT_Asym, N2c_Asym, N2c_latency_Asym, N2i_Asym, N2i_latency_Asym, CPPonset_Asym, CPPslope_Asym, AD_LI_SLF1, AD_LI_SLF2 ,AD_LI_SLF3,FA_LI_SLF1,FA_LI_SLF2,FA_LI_SLF3,MD_LI_SLF1,MD_LI_SLF2,MD_LI_SLF3,RD_LI_SLF1,RD_LI_SLF2,RD_LI_SLF3,VOL_LI_SLF1,VOL_LI_SLF2,VOL_LI_SLF3)
...
Accuracy_collapsed<-plyr::ddply(data2, .(ID, DAT1_3UTR), summarise, Accuracy=mean(Accuracy_overall))
...
num_trials2 <- plyr::ddply(data2, c("ID"), summarise,
...
RT_collapsed<-plyr::ddply(data2, .(ID, Hemifield, DAT1_3UTR, Site, DAT1_10_10_repeats), summarise, RT=mean(RT))
...
######Calculate RT asymmetry Index and add it to "participant_level"######
#Collapse each participant's trials accross ITI
RT_collapsed<-plyr::ddply(data2, .(ID, Hemifield, DAT1_3UTR), summarise, RT=mean(RT))
#Bring Target-Hemifield up into wide format
RT_collapsed <- reshape2::dcast(RT_collapsed, ID  ~ Hemifield, value.var="RT", fun.aggregate=mean)
RT_collapsed<-plyr::rename(RT_collapsed, c("Left"="RT_Left", "Right"="RT_Right"))
...
RT_Asym.mean <- plyr::ddply(RT_collapsed, "DAT1_3UTR", summarise, RT_Asym.mean=mean(RT_Asym))
RT_Asym.mean
